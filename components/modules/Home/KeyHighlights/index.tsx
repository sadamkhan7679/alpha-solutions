    ...
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
        {keyHighlights.highlights.map((item, index) => (
          <Card
            key={index}
            className="flex flex-col items-center justify-center px-4 py-8 bg-secondary rounded-lg"
          >
            {/*@ts-ignore*/}
            <CardHeader>
              {<item.Icon width={72} height={72} color="#fff" />}
            </CardHeader>
            <CardTitle className="text-5xl mb-5 text-white">
              {item.value}
            </CardTitle>
            <CardDescription className="text-3xl text-bold text-center text-white">
              {item.metric}
            </CardDescription>
          </Card>
        ))}
